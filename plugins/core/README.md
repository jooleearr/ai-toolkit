...
| `architecture-drift-watch` | `/core:architecture-drift-watch` (or model-invoked) | Scheduled, sceptical, architecture-level review of everything merged since the last run — the holistic decay across a window of history no single PR review could catch (boundary erosion, convergent duplication, abstraction decay, churn hotspots, co-change coupling, dependency creep, test rot, config/flag accumulation, security surface, doc drift). Reviews a window of commits against the repo's stated architecture, with git history as a first-class input. Files a labelled issue and posts one Slack message per run only when a finding clears the triage bar — silence is the common outcome. Ships `window`/`churn`/`co-change`/`test-skips` history scripts, a `PROJECTS.md` register, and reuses `pre-push-review`'s `CODE-SMELLS.md` at system altitude. |
