...
| `implement` | `/core:implement` (or model-invoked) | Implement a hand-off doc from the `plan` skill one vertical slice at a time, holding quality (testing, clean code, architecture, observability) as a constraint and proving the original problem solved. Dispatches the review agents below per slice for a structural pass, then offers to hand off to `pre-push-review` for the whole-change review rather than rolling into it silently. |
