...
description: Use when reviewing a change against its ticket and hand-off doc before pushing or opening a PR — checking it satisfies the acceptance criteria and honours the plan's agreed scope, non-goals, and assumptions, not just that the diff reads cleanly. Third skill in the plan → implement → pre-push review pipeline. Produces categorised, severity-ranked findings in Conventional Comments format with a ready-to-push verdict; delegates pure-diff bug-hunting to the code-review skill.
---
...
## 3. Review against the acceptance criteria
...
## 4. Review against scope, non-goals, and assumptions
...
## 5. Check batch size and slice integrity
...
## 6. Categorise, rank, and format the findings

Turn everything from steps 2–5 into findings. Each finding carries a **category**, a **severity**, and a single **Conventional Comments** line — see [`REVIEW-RUBRIC.md`](REVIEW-RUBRIC.md) for the category set, the severity scale, and the format with a worked example. Order the list **blocker-first**, so the reader triages the highest-impact items before any nit.
