...
| **Testing coverage** | Is each acceptance criterion actually exercised? Do the tests assert *behaviour* rather than implementation? What would still pass if the change were reverted? A coverage gap earns a finding only if the untested path can actually produce a user-visible defect — trace it to that failure first; a branch that degrades to correct behaviour is not a gap. |
